...
const AddTwitter = ({label, placeHolder}: {label: string, placeHolder: string}) => {
  return (
    <div className="w-full relative hidden md:block min-h-[150px]">
      <div className="w-full min-h-[100px] relative py-2 px-4">
        <div className="flex gap-4 justify-start items-center">
          <img
            ...
            alt="profile"
          />
          <AutoResizeTextarea placeHolder={placeHolder} />
        </div>
      </div>
      <div className="absolute right-3 bottom-3 gap-4 flex justify-end w-full items-center">
        <button className="px-10 py-2 bg-secondary-btn text-secondary-btn-text font-medium rounded-full">
          {label}
        </button>
      </div>
    </div>
